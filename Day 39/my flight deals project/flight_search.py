...
class FlightSearch:
    ...
    def __init__(self):
        ...
        self._token = self._get_new_token() #bearer token
        self._iata_code_endpoint = os.getenv("AMADEUS_IATA_CODE_ENDPOINT")
        self._flight_endpoint = os.getenv("AMADEUS_FLIGHT_ENDPOINT")

    def get_destination_code(self, city_name):
        # Manual override for known problematic cities
        city_overrides = {
            "Tokyo": "TYO",
            "Hong Kong": "HKG",
            "Kuala Lumpur": "KUL",
        ...
        # If city is in overrides, return it immediately
        if city_name in city_overrides:
            print(f"Using override IATA code for {city_name}: {city_overrides[city_name]}")
            return city_overrides[city_name]

        # Otherwise, call the Amadeus API
